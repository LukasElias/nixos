...
RowLayout {
    ...
    function pickPlayer(players) {
        for (let i = 0; i < players.length; i++) {
            if (players[i].dbusName == "org.mpris.MediaPlayer2.playerctld")
                return players[i];
        }
    ...
    readonly property string playingIcon: {
        if (!activePlayer)
            return "";
        if (playing)
            return String.fromCodePoint(0xf03e5); // Pause icon
        if (!playing)
            return String.fromCodePoint(0xf040c); // Play icon
    }
    readonly property string playerText: {
        if (!activePlayer)
            return "";
        if (trackArtist)
            return `${playingIcon} ${trackTitle} - ${trackArtist}`;
